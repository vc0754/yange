...
class AddressModel extends Model
{
    ...
    {
//        $user = UserModel::info();
        return self::where('user_id', 1)->select();
    }
    ...
    public static function edit($attributes)
    {
        ...
        if (isset($id)) {
            $model = self::where('id', $id)->where('user_id', $user->id)->find();
            // if (empty($model)) throw new Exception("不存在的地址");
            if (empty($model)) $model = new self;  //不存在的直接新增
        } else {
            $model = new self;
        }
