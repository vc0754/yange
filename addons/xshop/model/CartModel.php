...
class CartModel extends Model
{
    ...
    {
        $subQuery = \think\Db::table('__XSHOP_CART__')->alias('a')
            ->join('__XSHOP_PRODUCT_SKU__ b', 'a.sku_id=b.id', 'left')
            ->where('b.id is null')
            ->field('a.sku_id')
            ->buildSql();
        self::where("sku_id IN (SELECT * FROM $subQuery tmp)")->delete(); // 删除不存在的sku
        $user = UserModel::info();
