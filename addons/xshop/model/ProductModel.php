...
use addons\xshop\model\TermsModel;
...
class ProductModel extends Model
{
    ...
    {
        $user = UserModel::info();
        $product = self::with(['skus', 'category', 'unit', 'favorite' => function ($query) use ($user) {
    ...
    public static function getList($attributes)
    {
        ...
        $model = self::with(['skus', 'category'])->where('on_sale', 1);
        if (!empty($cat_id)) {
        ...
        if(isset($serviceTag)){
            $serviceTag = explode(',',$serviceTag);
            foreach ($serviceTag as $value){
                $model->where(function ($query) use ($value) {
                    return $query->whereOr('service_tags', 'like', "%$value%");
                });
            }
        }
        if (!isset($sort)) {
        ...
        switch ($sort) {
            ...
            case 5: {
                $model->order('fans', 'DESC');
                break;
            }
            case 6: {
                $model->order('fans', 'ASC');
                break;
            }
            case 7: {
                $model->order('favorite_count', 'DESC');
                break;
            }
            case 8: {
                $model->order('favorite_count', 'ASC');
                break;
            }
        }
    ...
    {
        return $this->belongsTo(CategoryModel::class, 'category_id', 'id', [], 'LEFT')->setEagerlyType(0);
    }
    ...
    public function getFansTextAttr($value, $data)
    {
        return empty($data['fans']) ? [] : ($data['fans'] < 1000 ? $data['fans'] : sprintf("%.1f ", $data['fans'] / 10000) . 'W');
    }


    public function getTermsAttr($value, $data)
    {

        $trems_id_list = empty($data['trems_ids']) ? [] : explode(',', $data['trems_ids']);
        $tremsList = [];
        $TermsModel = new TermsModel;
        foreach ($trems_id_list as $key => $value) {
            $data = $TermsModel->where('id', $value)->find();
            $tremsList[] = $data;
        }
        return $tremsList;
    }
    ...
    public function getNoticeAttr($value, $data)
    {
        ...
        $trems_id_list = empty($data['notice_ids']) ? [] : explode(',', $data['notice_ids']);
        $tremsList = [];
        $TermsModel = new TermsModel;
        foreach ($trems_id_list as $key => $value) {
            $data = $TermsModel->where('id', $value)->find();
