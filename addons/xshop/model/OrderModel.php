...
use app\admin\model\vc\Invoice;
...
class OrderModel extends Model
{
    ...
    public const ORDER_STATUS_DJD = 6; // 待接单
    public const ORDER_STATUS_CZZ = 7; // 创作中
    public const ORDER_STATUS_DFB = 8; // 待发布
    public const ORDER_STATUS_YFB = 9; // 已发布
    ...
    public static function getList(array $attributes = [])
    {
        ...
        switch ($state) {
            ...
            case 2: { // 待收货
                $model->where('status', 'in',[self::ORDER_STATUS_SHIPPED,self::ORDER_STATUS_DJD,self::ORDER_STATUS_CZZ,self::ORDER_STATUS_DFB,self::ORDER_STATUS_YFB]);
                break;
            }
            case 3: { // 待评价
    ...
    public static function add($attributes)
    {
        ...
        if (empty($list)) throw new NotFoundException("没有商品");
        $address = AddressModel::find($address_id);
        if (empty($address) || $address->user_id != 1) throw new NotFoundException("请选择收货地址");
        foreach ($list as $row) {
        ...
        try {
            $create_users = [];
            foreach($list as $k => $v) {
                $create_users[] = $v->product->create_user;
            }
            $user_ids = UserAccount::where('id', 'in', $create_users)->column('user_id');
            $data = [
                'order_sn' => $order_sn,
                'user_id' => $user->id,
                'creater_id' => implode(',', $user_ids),
            ...
            $Order->delivery_price = $service_price;
            $Order->products_price = $skuLogic->products_price;
            $Order->tax_price = isset($tax_price) ? $tax_price : 0;
            $Order->discount_price = $skuLogic->discount_price;

            $prices = isset($tax_price) ? bcadd($tax_price, $service_price, 2) : bcadd($service_price, 0, 2);
            $order_price = bcadd($prices, $skuLogic->order_price, 2);
    ...
    public static function buyOneInfo($attributes)
    {
        ...
        $product = ProductModel::with(['category'])->where('yg_xshop_product.id', $sku->product_id)->find();
    ...
    public static function cancel($attributes)
    {
        ...
        if ($invoice) {
            $invoice->delete_at = date('Y-m-d H:i:s');
            $invoice->save();
        }
    ...
    public function getStateTipAttr($value, $data)
    {
        ...
        switch ($status) {
            ...
            case self::ORDER_STATUS_SHIPPED:
                $val = '待收货';
                break;
            case self::ORDER_STATUS_DJD:
                $val = '待创作者接单';
                break;
            case self::ORDER_STATUS_CZZ:
                $val = '内容创作中';
                break;
            case self::ORDER_STATUS_DFB:
                $val = '内容待发布';
                break;
            case self::ORDER_STATUS_YFB:
                $val = '内容已发布';
                break;
