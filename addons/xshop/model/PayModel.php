...
class PayModel extends Model
{
    ...
    public static function notify()
    {
        ...
        //     'total_amount'  => 0,
        //     'total_fee'     => 0,
        //     'out_trade_no'  => 'O2021091648484851',
        // ];
        ...
        try {
            ...
            $order = OrderModel::with([ 'products' ])->where(['order_sn' => $order_sn])->find();
            if (!empty($order)) {
                $order->status = 1;
                $order->is_pay = 1;
            ...
            foreach($order['products'] as $sku) {
                ...
                $Account = UserAccount::find($Product->create_user);
                ...
                if ($WeixinVender) {
                    $data = [
                        ...
                        'template_id'   => 'Dzn-8EmnIsHZHQ-uBY4h-1kBunbbPzH3MJtgeGNb8aM',
                        'miniprogram'   => [
                        ...
                        'data'          => [
                            'first'     => [
                                'value' => '您有一笔新的订单，请尽快处理'
                            ],
                            'keyword1'  => [
                                'value' => $order_sn
                            ],
                            'keyword2'  => [
                                'value' => '『' . $sku['title'] . '』 ' . $sku['attributes'] . '服务'
                            ],
                            'keyword3'  => [
                                'value' => $sku['price'] . '元'
                            ],
                            'keyword4'  => [
                                'value' => date('Y年m月d日 H:i:s', $order->pay_time)
                            ],
                            'remark'    => [
                                'value' => '要求发布时间为：' .  date('Y年m月d日', strtotime($order->fbTime)) . '，请尽快处理'
                            ],
                        ]
                    ];
