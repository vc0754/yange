    ...
    var Controller = {
        ...
        bootIndex: function () {
                ...
                columns: [
                    [
                        ...
                        {field: 'status', title: __('Status'), formatter: function(val, row) {
                                ...
                                else if (val == 6) tip += '<div style="color: #3c8dbc;">待创作者接单</div>'
                                else if (val == 7) tip += '<div style="color: #3c8dbc;">内容创作中</div>'
                                else if (val == 8) tip += '<div style="color: #3c8dbc;">内容待发布</div>'
                                else if (val == 9) tip += '<div style="color: #3c8dbc;">内容已发布</div>'
                        ...
                        // }},
                        {field: 'delivery_price', title: '平台服务费', operate:'BETWEEN', visible: true},
                        {field: 'order_price', title: __('Order_price'), operate:'BETWEEN', visible: true},
                        {field: 'products_price', title: __('Products_price'), operate:'BETWEEN', visible: true},
                        {field: 'tax_price', title: '税费', operate:'BETWEEN'},
                        {field: 'discount_price', title: __('Discount_price'), operate:'BETWEEN', visible: true},
                        ...
                        {field: 'fbTime', title: __('发布时间'), operate:'BETWEEN'},
                        {field: 'material', title: __('素材链接'), operate:'BETWEEN'},
                        ...
                        {field: 'operate', title: __('Operate'), table: table, events: Table.api.events.operate, formatter: Table.api.formatter.operate, buttons: [
                            ...
                            {
                                name: 'pay', text: '待创作者接单', icon: '', classname: 'btn btn-xs btn-info btn-djd',
                                dropdown: '操作',
                                visible: function(row) {
                                    if (row.status != -1 && row.is_pay == 1 && row.is_delivery == 0) return true
                                    return false
                                }
                            },
                            {
                                name: 'pay', text: '内容创作中', icon: '', classname: 'btn btn-xs btn-info btn-czz',
                                dropdown: '操作',
                                visible: function(row) {
                                    if (row.status != -1 && row.is_pay == 1 && row.is_delivery == 0) return true
                                    return false
                            ...
                            {
                                name: 'pay', text: '内容待发布', icon: '', classname: 'btn btn-xs btn-info btn-dfb',
                                dropdown: '操作',
                                visible: function(row) {
                                    if (row.status != -1 && row.is_pay == 1 && row.is_delivery == 0) return true
                                    return false
                            ...
                            {
                                name: 'pay', text: '内容已发布', icon: '', classname: 'btn btn-xs btn-info btn-yfb',
                                dropdown: '操作',
                                visible: function(row) {
                                    if (row.status != -1 && row.is_pay == 1 && row.is_delivery == 0) return true
                                    return false
            ...
            table.on('post-body.bs.table', function(e, setting, json, xhr) {
                ...
                $('.btn-djd', this).on('click', function(e) {
                    var row = table.bootstrapTable('getData')[$(this).data('row-index')]
                    Layer.confirm('确定将订单设置为“待创作者接单”吗？', {icon: 3, title: '提示'}, function(index) {
                        Fast.api.ajax({
                            url: 'xshop/order/updateStatus',
                            data: {
                                id: row.id,
                                status:6
                            }
                ...
                $('.btn-czz', this).on('click', function(e) {
                    var row = table.bootstrapTable('getData')[$(this).data('row-index')]
                    Layer.confirm('确定将订单设置为“内容创作中”吗？', {icon: 3, title: '提示'}, function(index) {
                        Fast.api.ajax({
                            url: 'xshop/order/updateStatus',
                            data: {
                                id: row.id,
                ...
                $('.btn-dfb', this).on('click', function(e) {
                    var row = table.bootstrapTable('getData')[$(this).data('row-index')]
                    Layer.confirm('确定将订单设置为“内容待发布”吗？', {icon: 3, title: '提示'}, function(index) {
                        Fast.api.ajax({
                            url: 'xshop/order/updateStatus',
                            data: {
                                id: row.id,
                ...
                $('.btn-yfb', this).on('click', function(e) {
                    var row = table.bootstrapTable('getData')[$(this).data('row-index')]
                    Layer.confirm('确定将订单设置为“内容已发布”吗？', {icon: 3, title: '提示'}, function(index) {
                        Fast.api.ajax({
                            url: 'xshop/order/updateStatus',
                            data: {
                                id: row.id,
